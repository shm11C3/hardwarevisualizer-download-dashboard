...
- レスポンスは Workers Cache（`wrangler.jsonc` の `cache.enabled`）が Cloudflare のエッジでキャッシュし、ヒット時は Worker を起動せずに返します。対象はレスポンスの `Cache-Control` で制御しており、`/`・`/api/dashboard`・`/api/export.csv` の成功レスポンスが最大5分（+ stale-while-revalidate 10分）キャッシュされます。キャッシュキーはパスとクエリ文字列（パラメータの順序も区別）で、デプロイバージョンごとに独立するため、デプロイ直後は空のキャッシュから始まります。画面の「更新」リンクが付与する `t` パラメータは URL を一意にし、そのレスポンス自体も `no-store` になるため、従来どおりキャッシュをバイパスします。ヒット状況はレスポンスの `Cf-Cache-Status` ヘッダで確認できます。
